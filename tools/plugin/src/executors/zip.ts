...
const runExecutor: PromiseExecutor<ZipExecutorSchema> = async (options, context: ExecutorContext) => {
  ...
  try {
    ...
    execSync(`cd ${builtProjectPath} && find . -name "*.js" -o -name "package.json" -o -name "plugin.manifest.json" | zip -r ${zipFilePath} -@`, {
      cwd: root,
      stdio: 'inherit',
    });
